 ...
 * 
 * @param {string} a numbers only
 * @param {string} op operation (+, -)
 * @param {string} b numbers only
 * @return {string} result as number string, single leading '-' if negative
 */
exports.calc = function(a, op, b) {
    var zrx = /^0+/;
    a = a.match(/\d/g).join('').replace(zrx, '');
    b = b.match(/\d/g).join('').replace(zrx, '');
    if (op === '+')
        return sum(a, b);
    if (op === '-')
        return subtract(a, b);
 ...
 * 
 * @param {string} a positive, numbers only
 * @return {string} result as number string, single leading '-' if negative
 */
